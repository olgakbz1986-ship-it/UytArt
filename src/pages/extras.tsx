...
export function HowItWorksPage() {
  const buyerSteps = [
    { icon: UserPlus, title: "Регистрация", text: "Создайте аккаунт за 30 секунд — email и пароль. Никаких анкет и подтверждений." },
    { icon: Search, title: "Поиск мастера", text: "Фильтры по городу, категории, рейтингу. Или умный поиск по описанию и фото." },
    { icon: ShoppingBag, title: "Выбор и заказ", text: "Детальные карточки с историями мастеров, отзывами и видео. Корзина и оформление в 2 клика." },
    { icon: CreditCard, title: "Безопасная оплата", text: "Деньги замораживаются до получения товара. ЮKassa, карты, СБП — выбирайте удобное." },
    { icon: PackageCheck, title: "Получение и отзыв", text: "Отслеживание доставки, подтверждение получения, честный отзыв мастеру." },
  ];
  const masterSteps = [
    { icon: UserPlus, title: "Регистрация мастера", text: "Заполните профиль: история мастерской, фото, декларация качества. Модерация за 24 часа." },
    { icon: Palette, title: "Создание витрины", text: "Загрузите товары с описаниями и фото. AI поможет сгенерировать карточки за минуту." },
    { icon: BarChart3, title: "Аналитика и рост", text: "Отслеживайте просмотры, конверсию, выручку. ИИ-аналитик даёт персональные советы." },
    { icon: TrendingUp, title: "Продажи и выплаты", text: "Принимайте заказы, отправляйте товары, получайте выплаты на карту раз в неделю." },
    { icon: Award, title: "Развитие бренда", text: "Рейтинг из реальных отзывов, мини-сайт мастерской, участие в акциях платформы." },
  ];
  return (
    <div>
      <section className="relative overflow-hidden bg-dark text-cream py-20">
        <div className="absolute inset-0" style={{ backgroundImage: "radial-gradient(circle at 20% 30%, rgba(217,142,50,0.15) 0, transparent 40%), radial-gradient(circle at 80% 70%, rgba(45,95,76,0.3) 0, transparent 45%)" }} />
        <div className="relative max-w-[900px] mx-auto px-5 text-center">
          <h1 className="font-display font-extrabold text-[40px] leading-tight mb-4">Как работает УютАрт</h1>
          <p className="text-[16px] text-cream/80 max-w-[600px] mx-auto">Маркетплейс авторских вещей, где мастера и покупатели находят друг друга без посредников и алгоритмов.</p>
        </div>
      </section>

      <section className="max-w-[900px] mx-auto px-5 py-16">
        <h2 className="font-display font-bold text-[28px] text-ink mb-8 text-center">Для покупателей</h2>
        <div className="space-y-6">
          {buyerSteps.map((s, i) => (
            <div key={i} className="flex gap-5 items-start bg-surface rounded-2xl shadow-card p-6">
              <div className="shrink-0 w-12 h-12 rounded-xl bg-accent-deep/10 flex items-center justify-center">
                <s.icon size={22} className="text-accent-deep" />
              </div>
              <div>
                <p className="font-display font-bold text-[18px] text-ink mb-1">{i + 1}. {s.title}</p>
                <p className="text-[14px] text-ink-soft leading-relaxed">{s.text}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="bg-cream py-16">
        <div className="max-w-[900px] mx-auto px-5">
          <h2 className="font-display font-bold text-[28px] text-ink mb-8 text-center">Для мастеров</h2>
          <div className="space-y-6">
            {masterSteps.map((s, i) => (
              <div key={i} className="flex gap-5 items-start bg-surface rounded-2xl shadow-card p-6">
                <div className="shrink-0 w-12 h-12 rounded-xl bg-accent-deep/10 flex items-center justify-center">
      ...
      <section className="max-w-[700px] mx-auto px-5 py-16 text-center">
        <h2 className="font-display font-bold text-[24px] text-ink mb-4">Готовы начать?</h2>
        <p className="text-[15px] text-ink-soft mb-6">Присоединяйтесь к сообществу мастеров и ценителей авторских вещей.</p>
        <div className="flex gap-3 justify-center flex-wrap">
          <Link to="/register"><Btn>Я покупатель</Btn></Link>
          <Link to="/seller/register"><Btn variant="outline">Я мастер</Btn></Link>
        </div>
      </section>
    </div>
  );
